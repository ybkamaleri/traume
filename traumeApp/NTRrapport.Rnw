...
Antall og andel pasienter med NISS under og over 15.\\
...
Antall og andel pasienter med ISS under og over 15.\\
...
Figur \ref{fig:ukedager} viser fordeling av antall og andel pasienter fordelt på ukedager i akuttmottak.\\
...
Gjennomsnitt liggedøgn er \Sexpr{meanUT} dager. Tabellen nedenfor viser fordelingen av antall dager av liggedøgn og antall pasienter.\\


<<ligge, resultat='asis'>>=
liggTab <- intDT[!duplicated(ntrid) & !is.na(hosp_icu_days), .N, by = hosp_icu_days]
setkey(liggTab, hosp_icu_days)
data.table::setnames(liggTab, c("hosp_icu_days", "N"), c("Antall Dager", "Antall traume"))
